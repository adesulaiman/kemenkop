...
<script>
  ...
  var table = $("#<?php echo $formName ?>").DataTable({
    ...
    buttons: [
      ...
      {
        text: '<i class="fa fa-file-excel-o"></i> Upload Excel',
        action: function(e, dt, node, config) {
          var rowData = dt.rows(".selected").data()[0];

          $('#ModalText<?php echo $formName ?>').text("Upload Data by Excel");
          $('.formModal<?php echo $formName ?>').html("<div class='form-group col-md-3'>\
                                    <label for='transkip_uplaod'>\
                                        Upload Data Usaha<br>\
                                        <img src='assets/img/add-image2.png' style='width:60%'  class='data-pemilik-usaha-img imgreg'/>\
                                        <br><span class='data-pemilik-usaha-info'></span>\
                                    </label>\
                                    <input type='file' accept='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' class='form-control data-pemilik-usaha-input' name='data-pemilik-usaha_upload'/>\
                                </div><div class='row'></div>\
                                <span><b>Untuk upload data usaha menggunakan excel, silakan anda download tempate data usaha berupa data zip yang berisi :</b><br>\
                                1. data_pemilik_usaha.xlsx (excel yang harus di isi dengan data pemilik usaha)<br>\
                                2. data_referensi_pemilik_usaha.xlsx (excel yang berisi data referensi, seperti id provinsi dan lain lain)<br>\
                                3. ketika data_pemilik_usaha.xlsx sudah di isi, silakan di upload, dan click upload<br><br></span>\
                                <a href='./assets/upload/template_data_pemilik_usaha.zip' class='btn btn-success'>Download Template Excel</a>");

          imageInit($(".data-pemilik-usaha-img"), $(".data-pemilik-usaha-input"), $(".data-pemilik-usaha-info"));

          $('.actFilter').css('display', 'none');
          $('.formSubmit').css('display', 'none');
          $('.formUpload').css('display', 'inline');
